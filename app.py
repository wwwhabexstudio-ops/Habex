...
def search_pexels_video(query, orientation):
    ...
    # Prefer an HD file over 4K (faster download, plenty good enough).
    files_sorted = sorted(files, key=lambda f: (f.get("quality") != "hd", f.get("width", 0)))
...
def generate_stock_footage_video(script, aspect_ratio, voice, add_captions, progress=gr.Progress()):
    ...
    try:
        for i, scene in enumerate(scenes):
            ...
            stock_path = search_pexels_video(query, orientation)
            if aspect_ratio and aspect_ratio != "Native (no cropping)":
                stock_path = apply_aspect_ratio(stock_path, aspect_ratio)
            ...
            matched = loop_video_to_duration(video_clip, audio_clip.duration)
            scene_clips.append(matched.with_audio(audio_clip))
    except Exception as e:
        for c in scene_clips:
            c.close()
        log_video_finished(video_id, "failed", error_message=e)
        raise gr.Error(f"Stock footage assembly failed partway through (scene {len(scene_clips) + 1}): {e}")
