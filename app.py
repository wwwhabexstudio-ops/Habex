...
def generate_stock_footage_video(script, aspect_ratio, voice, add_captions, progress=gr.Progress()):
    ...
    # crash on Render's free-tier memory limit. This does mean one extra
    # encode pass per scene (a bit slower) in exchange for not crashing.
    scene_paths = []
    try:
        for i, scene in enumerate(scenes):
            ...
            cropped = crop_clip_to_aspect(video_clip, aspect_ratio)
            matched = loop_video_to_duration(cropped, audio_clip.duration)
    ...
    concat_clips = []
    try:
        concat_clips = [VideoFileClip(p) for p in scene_paths]
