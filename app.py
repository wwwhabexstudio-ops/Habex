...
def generate_scene_image(visual_prompt, style):
    ...
    # NOTE: verify exact param/endpoint names via the Space's "Use via API"
    # page (huggingface.co/spaces/black-forest-labs/FLUX.1-schnell) — this
    # is the same kind of API-drift risk flagged for the video backend.
    result = client.predict(full_prompt, api_name="/infer")
...
def generate_video(
    ...
    try:
        for i in range(num_clips):
            ...
            # Visit https://huggingface.co/spaces/Lightricks/ltx-video-distilled
...
with gr.Blocks(title="Habex Studio", theme=theme, css=CUSTOM_CSS) as demo:
    ...
    # Also drop the script straight into the caption/narration/metadata
    # boxes, since that's the most common thing someone wants to reuse it for.
    send_to_video_btn.click(fn=lambda s: s, inputs=script_output, outputs=caption_text)
    send_to_video_btn.click(fn=lambda s: s, inputs=script_output, outputs=narration_text)
    ...
    # Actually switch to the Generate Video tab instead of just filling fields.
    send_to_video_btn.click(fn=lambda: gr.Tabs(selected="tab_video"), outputs=main_tabs)
    ...
    video_send_btn.click(fn=lambda v: v, inputs=video_output, outputs=publish_file)
    video_send_btn.click(fn=lambda: gr.Tabs(selected="tab_publish"), outputs=main_tabs)
    ...
    meta_send_btn.click(
        fn=parse_metadata_for_publish,
        inputs=meta_output,
        outputs=[publish_title, publish_description, publish_tags],
    )
    meta_send_btn.click(fn=lambda: gr.Tabs(selected="tab_publish"), outputs=main_tabs)
    ...
    sd_send_btn.click(fn=lambda v: v, inputs=sd_output, outputs=publish_file)
    sd_send_btn.click(fn=lambda: gr.Tabs(selected="tab_publish"), outputs=main_tabs)
