    ...
    for model_name in candidates:
        try:
            ...
            if e.response is not None and e.response.status_code == 404:
                # This one's listed as available but doesn't actually work
                # (a real, documented Gemini API inconsistency) — remember
                # that and move on to the next candidate instead of retrying it.
        ...
        raise RuntimeError(
            f"None of this key's available models actually work for "
            f"generateContent (tried: {candidates}). Last error: {last_error}"
        )
    try:
...
def generate_script(topic, video_length_minutes, tone):
    ...
    try:
        ...
        # viewers hanging.
        if script and script.rstrip()[-1] not in ".!?\"'":
            finish_prompt = (
            ...
            script = f"{script.rstrip()} {ending.strip()}"
