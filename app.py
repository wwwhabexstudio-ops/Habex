...
def generate_stock_footage_video(script, aspect_ratio, voice, add_captions, progress=gr.Progress()):
    ...
    try:
        for i, scene in enumerate(scenes):
            ...
            # Crop in-memory (no intermediate write/encode) — the whole
            # clip gets encoded once anyway when the final video is written.
            video_clip = crop_clip_to_aspect(video_clip, aspect_ratio)
    ...
    try:
        ...
        # "ultrafast" trades some file size for meaningfully quicker
        # encoding — worth it on Render's free-tier CPU.
