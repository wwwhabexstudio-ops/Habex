...
# Google's Gemini API has a genuine permanent free tier (no credit card,
# no trial expiry) — noticeably stronger writing quality than the small
# open model this used to run on. Get a free key at aistudio.google.com/apikey
GEMINI_API_KEY = os.environ.get("GEMINI_API_KEY")

# Kept as a fallback reference only — no longer the primary text model.
HF_TOKEN = os.environ.get("HF_TOKEN")

_gemini_model_cache = None
_gemini_known_broken = set()


def get_gemini_model_candidates():
    """Ranked list of every model this key can use for generateContent,
    best choice first. A ranked LIST rather than a single pick matters
    here — Google's model-listing endpoint has a documented, real
    inconsistency where a model shows as available but 404s when actually
    called, so the caller needs somewhere to fall back to."""
    resp = requests.get(
    ...
    if not candidates:
        raise RuntimeError("This Gemini API key has no models available for generateContent.")
    # Prefer stable "flash" models (fast, generous free tier) over
    # pro/preview/experimental ones, which tend to be lower-quota or unstable.
    stable_flash = [
        c for c in candidates
        if "flash" in c and not any(x in c for x in ("lite", "preview", "exp"))
    ]
    rest = [c for c in candidates if c not in stable_flash]
    return stable_flash + rest


def call_gemini(system_prompt, user_prompt, max_tokens=2000, temperature=0.9):
    if not GEMINI_API_KEY:
        raise RuntimeError(
            "No Gemini API key configured. Create one free at "
            "aistudio.google.com/apikey and add it as GEMINI_API_KEY."
        )
    ...
    def _request_with_retry(model_name, max_retries=2):
        # 5xx means Google's servers are temporarily overloaded, not that
        # anything is wrong with the request — worth a couple of quick
        # retries before giving up or switching models.
        for attempt in range(max_retries + 1):
            try:
                return _request(model_name)
    ...
    candidates = [_gemini_model_cache] if _gemini_model_cache else []
    candidates += [c for c in get_gemini_model_candidates() if c not in candidates]
    candidates = [c for c in candidates if c not in _gemini_known_broken] or candidates

    last_error = None
    for model_name in candidates:
        try:
            data = _request_with_retry(model_name)
            _gemini_model_cache = model_name
            break
        except requests.exceptions.HTTPError as e:
            status = e.response.status_code if e.response is not None else None
            if status == 404:
                # Listed as available but doesn't actually work (a real,
                # documented Gemini API inconsistency) — never try it again.
                _gemini_known_broken.add(model_name)
            # Any HTTP error (404, or a 5xx that survived retries) — try
            # the next candidate instead of failing the whole call outright.
            last_error = e
            continue
        except (requests.exceptions.Timeout, requests.exceptions.ConnectionError) as e:
            # Same idea for network-level failures — a stalled connection
            # to one model shouldn't take down the whole request when
            # another model might respond fine.
            last_error = e
            continue
    else:
        raise RuntimeError(
            f"None of this key's available models responded successfully "
...
def is_video_blank(video_path, std_threshold=8.0):
    """Detect a near-solid-color/blank video. The free LTX backend has
    been observed to occasionally return a technically valid file with
    no actual visual content — no error raised, just an empty result
    that would otherwise look like a silent success. Checks pixel
    variance at three points across the clip rather than just one frame,
    in case only part of a generation came out blank."""
    clip = VideoFileClip(video_path)
    try:
        checkpoints = [clip.duration * f for f in (0.1, 0.5, 0.9)]
        return all(clip.get_frame(t).std() < std_threshold for t in checkpoints)
    finally:
        clip.close()
...
def generate_stock_footage_video(script, aspect_ratio, voice, add_captions, progress=gr.Progress()):
    ...
    try:
        for i, scene in enumerate(scenes):
            ...
            query = scene.get("stock_keywords") or scene.get("visual") or scene.get("narration", "")
            progress(pct, desc=f'Scene {i + 1}/{len(scenes)}: searching "{query}"...')
...
def generate_script(topic, video_length_minutes, tone):
    ...
    system_prompt = (
        "You are a professional scriptwriter for short-form video, writing "
        "the way a real, slightly informal human creator talks out loud — "
        "not the way an AI assistant writes. Follow every rule:\n"
        "1. HOOK: the first line must be a scroll-stopping hook — a bold "
        "claim, a surprising fact, a real question, or genuine tension. "
        "Never open with a greeting or 'today we're talking about'.\n"
        "2. RETENTION: use open loops and pattern interrupts every few "
        "lines so there's no natural point to swipe away.\n"
        "3. COMPLETE ENDING — critical: the script must reach a real "
        "conclusion or payoff line. Never trail off mid-thought, never "
        "stop before an idea resolves. If you're running long, wrap up "
        "cleanly rather than cutting off.\n"
        "4. SOUND HUMAN, NOT AI-GENERATED: never use the 'it's not X, "
        "it's Y' contrast trick, filler ('it's important to note', 'in "
        "today's world'), inflated-significance phrases ('plays a vital "
        "role', 'game-changer'), rule-of-three lists, or generic hype "
        "words ('unlock', 'dive into', 'elevate'). Vary sentence length "
        "unevenly, like someone actually talking — not uniform, tidy "
        "sentences. Use specific, concrete details, never generic claims.\n"
        "5. Short, punchy sentences overall. No throat-clearing.\n"
    ...
    user_prompt = (
        ...
        "every rule above — especially the hook and the complete ending."
    )
    max_tokens = min(4000, int(target_words * 3))

    try:
        script = call_gemini(system_prompt, user_prompt, max_tokens=max_tokens)

        # Two independent quality checks, each with a single corrective
        # follow-up call rather than silently accepting a bad result.
        ...
        if word_count < target_words * 0.6:
            expand_prompt = (
                ...
                f"same rules, don't repeat what's already written, and make "
                f"sure it reaches a real ending:\n\n{script}"
            )
            continuation = call_gemini(system_prompt, expand_prompt, max_tokens=max_tokens)
            script = f"{script}\n\n{continuation}"

        # If it doesn't end on terminal punctuation, it likely got cut off
        # mid-thought — ask for a proper closing line rather than leaving
        # viewers hanging.
        stripped_script = script.rstrip() if script else ""
        if stripped_script and stripped_script[-1] not in ".!?\"'":
            finish_prompt = (
                f"This script cuts off before finishing:\n\n{script}\n\n"
                "Write ONLY the final 1-3 sentences that complete it with "
                "a real payoff or closing line — same voice, same rules. "
                "Do not repeat any part of the script above."
            )
            ending = call_gemini(system_prompt, finish_prompt, max_tokens=200)
            script = f"{stripped_script} {ending.strip()}"

        return script
    except Exception as e:
        raise gr.Error(
            f"Script generation failed (Gemini's free tier may be busy "
            f"or rate-limited right now): {e}"
        )
...
def generate_scene_plan(script):
    ...
    system_prompt = (
        ...
        "give three things:\n"
        "1. 'narration': the exact narration text for that scene (a "
        "verbatim slice of the original script — do not paraphrase).\n"
        "2. 'visual': a vivid, concrete visual description an AI image "
        "generator can render (specific subject, setting, action — "
        "never an abstract concept like 'financial stress').\n"
        "3. 'stock_keywords': 2-4 SHORT literal keywords for searching a "
        "stock footage library (e.g. 'man counting cash', 'empty wallet "
        "closeup', 'city skyline sunset') — concrete, filmable, generic "
        "enough that real stock footage of it plausibly exists. Never "
        "abstract, never a full sentence.\n"
        "Respond with ONLY a JSON array, no commentary, no markdown "
        'fences, in exactly this shape: '
        '[{"narration": "...", "visual": "...", "stock_keywords": "..."}, ...]'
    )
    user_prompt = f"Script:\n{script}\n\nBreak it into scenes now."
    try:
        raw = call_gemini(system_prompt, user_prompt, max_tokens=2000)
    except Exception as e:
        raise gr.Error(f"Scene planning failed (Gemini's free tier may be busy): {e}")
...
# fal.ai gives one-time signup credit (no card required) but NO renewing
# free tier — once spent, every call costs real money. Using the cheaper
# 480p tier (~$0.20/clip) rather than the 720p "Pro" model (~$0.80/clip)
# to stretch that one-time credit as far as possible.
FAL_API_KEY = os.environ.get("FAL_KEY")


def animate_image_with_fal(image_path, motion_prompt):
    """Real AI-generated motion (Wan 2.1) instead of pan/zoom. Uses
    fal.ai's one-time signup credit — will eventually run out and start
    costing real money, unlike everything else in this app."""
    if not FAL_API_KEY:
        raise RuntimeError("fal.ai isn't connected — missing FAL_KEY on the server.")
    ...
    out_path = os.path.join(tempfile.gettempdir(), f"fal_motion_{uuid.uuid4().hex}.mp4")
...
def generate_scene_director_video(
    script, style, aspect_ratio, voice, add_captions, character_description="",
    use_ai_motion=False, progress=gr.Progress(),
):
    if not script or not script.strip():
        raise gr.Error("Paste or write a script first — Tab 1 can generate one.")
    ...
    try:
        for i, scene in enumerate(scenes):
            ...
            if use_ai_motion:
                try:
                    progress(pct, desc=f"Scene {i + 1}/{len(scenes)}: generating AI motion (fal.ai)...")
                    motion_path = animate_image_with_fal(
                        image_path, scene.get("visual") or scene.get("narration", "")
                    )
                    raw_clip = VideoFileClip(motion_path)
                    resized = crop_and_resize_to_size(raw_clip, size)
                    visual_clip = loop_video_to_duration(resized, audio_clip.duration)
                except Exception as e:
                    # Trial credit exhausted, rate-limited, or any other
                    # failure — fall back to the free method for this
                    # scene rather than aborting the whole video.
                    gr.Warning(f"Scene {i + 1}: AI motion failed ({e}) — used pan/zoom instead.")

            if visual_clip is None:
                visual_clip = image_to_kenburns_clip(image_path, audio_clip.duration, size)
...
def generate_metadata(content, platforms):
    ...
    for platform in platforms:
        ...
        system_prompt = (
            "You write high-converting video metadata for social "
            "platforms, in the voice of a real creator — not generic "
            "marketing copy. Follow the given platform's conventions "
            "exactly. Never use filler like 'check out this video', "
            "never use inflated hype words ('game-changer', 'unlock', "
            "'must-watch'), never write like an ad. Be specific to what "
            "this particular video is actually about."
        )
        user_prompt = (
        ...
        try:
            sections.append(f"### {platform}\n{call_gemini(system_prompt, user_prompt, max_tokens=500)}")
        except Exception as e:
            sections.append(f"### {platform}\n(generation failed: {e})")
...
def generate_video(
    ...
    try:
        for i in range(num_clips):
            ...
            if is_video_blank(clip):
                raise RuntimeError(
                    "The backend returned a blank/empty clip (no visible "
                    "content) — this happens occasionally on the free "
                    "tier. Try generating again."
                )
...
def submit_publish_request(uploaded_file, platforms, title, description, tags, caption, schedule_dt):
    ...
    scheduled_iso = None
    if schedule_dt and schedule_dt.strip():
        raw = schedule_dt.strip()
        try:
            # Accept "2026-09-01T12:00:00" (and a few close variants);
            # reject anything else clearly instead of silently sending a
            # broken value to the database.
            parsed = datetime.fromisoformat(raw.replace("Z", "+00:00"))
            scheduled_iso = parsed.isoformat()
        except ValueError:
            raise gr.Error(
                f"'{raw}' isn't a valid date/time. Use this exact format: "
                f"2026-09-01T14:00:00 (year-month-dayThour:minute:second)."
            )
...
with gr.Blocks(title="Habex Studio", theme=theme, css=CUSTOM_CSS) as demo:
        ...
        with gr.Tab("3. Scene Director (Beta)", id="tab_scene"):
            ...
            with gr.Row():
                with gr.Column():
                    ...
                    sd_ai_motion = gr.Checkbox(
                        label="Use real AI motion (fal.ai) instead of pan/zoom",
                        value=False,
                        info=(
                            "⚠️ Uses fal.ai's one-time signup credit, NOT a "
                            "renewing free tier like everything else in this "
                            "app — each scene costs real credit (~$0.20), "
                            "and once it's spent this starts charging real "
                            "money. Leave unchecked to keep this tab fully "
                            "free (pan/zoom motion). Falls back to pan/zoom "
                            "automatically if a scene's credit call fails."
                        ),
                    )
    ...
    sd_btn.click(
        ...
        inputs=[sd_script, sd_style, sd_aspect, sd_voice, sd_captions, sd_character, sd_ai_motion],
        outputs=sd_output,
    )
    sd_send_btn.click(
        fn=send_video_to_publish_tab,
