...
def get_gemini_model():
    """Google deprecates/renames Gemini model versions frequently — asking
    the API which models this key can actually use, instead of hardcoding
    a version name that may not exist by the time this runs, avoids
    breaking every time Google reshuffles their lineup."""
    ...
    # Prefer a stable "flash" model (fast, generous free tier) over
    ...
    _gemini_model_cache = stable_flash[0] if stable_flash else candidates[0]
    return _gemini_model_cache
...
def call_gemini(system_prompt, user_prompt, max_tokens=2000, temperature=0.9):
    ...
    try:
        ...
        if e.response is not None and e.response.status_code == 404:
            # Cached model name went stale (Google deprecated it mid-session)
            # — refresh the list and retry once with whatever's current now.
            _gemini_model_cache = None
            model_name = get_gemini_model()
            data = _request(model_name)
        else:
